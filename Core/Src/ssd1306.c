...
void SSD1306_DrawBitmap(int16_t x, int16_t y, const unsigned char* bitmap, int16_t w, int16_t h, uint16_t color) {
    int16_t byteWidth = (w + 7) >> 3; // Bitwise division by 8
    uint8_t byte = 0;
    ...
    for(int16_t j=0; j<h; j++, y++) {
        for(int16_t i=0; i<w; i++) {
            if(i & 7) {
               ...
               byte = (*(const unsigned char *)(&bitmap[j * byteWidth + (i >> 3)])); // Bitwise
            }
...
uint8_t SSD1306_Init(void) {
	...
	/* Use reliable HAL Delay instead of blocking loop */
	HAL_Delay(5);
...
void SSD1306_ToggleInvert(void) {
	...
	/* 4x Faster memory toggle using 32-bit operations */
	uint32_t *ptr = (uint32_t *)SSD1306_Buffer;
	for (uint16_t i = 0; i < (sizeof(SSD1306_Buffer) / 4); i++) {
		ptr[i] = ~ptr[i];
	}
}
...
void SSD1306_DrawPixel(uint16_t x, uint16_t y, SSD1306_COLOR_t color) {
	...
	/* Optimized bitwise calculations for speed */
	uint16_t index = x + (y >> 3) * SSD1306_WIDTH; // equivalent to y / 8
	uint8_t bitMask = 1 << (y & 7);                // equivalent to y % 8

	if (color == SSD1306_COLOR_WHITE) {
		SSD1306_Buffer[index] |= bitMask;
	} else {
		SSD1306_Buffer[index] &= ~bitMask;
	}
}
...
void ssd1306_I2C_Init() {
    /* Safe non-blocking delay if initialization delay is required */
    HAL_Delay(10);
}

/*
 * CRITICAL OPTIMIZATION:
 * Used HAL_I2C_Mem_Write directly instead of wasting RAM creating a 256-byte
 * local buffer (dt[256]) and slow copying in a for loop.
 */
void ssd1306_I2C_WriteMulti(uint8_t address, uint8_t reg, uint8_t* data, uint16_t count) {
    HAL_I2C_Mem_Write(&hi2c2, address, reg, I2C_MEMADD_SIZE_8BIT, data, count, 100);
}

/* Optimized for consistency and slight speed boost */
void ssd1306_I2C_Write(uint8_t address, uint8_t reg, uint8_t data) {
    HAL_I2C_Mem_Write(&hi2c2, address, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, 10);
}
